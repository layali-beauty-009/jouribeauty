...
export function FinalCTA({
  ...
  return (
    <section className="px-4 py-14 sm:py-16">
      <div className="relative mx-auto max-w-lg overflow-hidden rounded-3xl bg-teal-dark p-8 text-center text-pearl shadow-xl ring-1 ring-gold/25 sm:max-w-2xl md:p-12 lg:max-w-4xl">
        <div
        ...
        <div className="relative">
          <span className="text-[11px] font-bold tracking-[0.28em] text-gold">{label}</span>
          <h2 className="mx-auto mt-4 max-w-lg font-sans text-xl font-extrabold leading-snug md:text-2xl lg:text-[1.75rem]">
            {title}
          </h2>
          ...
          {chips && chips.length > 0 && (
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {chips.map((chip) => (
                <span
                  key={chip.text}
                  className="inline-flex items-center gap-1.5 rounded-full border border-gold/35 bg-gold/10 px-3 py-1.5 text-[11px] font-semibold text-pearl"
                >
